...
public class ClassesProcessor {
  ...
  public void writeClass(StructClass cl, TextBuffer buffer) throws IOException {
    ...
    // Special debugging for l2/gameserver/model/Creature
    if ("l2/gameserver/model/Creature".equals(cl.qualifiedName)) {
      DecompilerContext.getLogger().writeMessage(
        "=== DEBUG: Starting writeClass for problematic class: " + cl.qualifiedName + " ===", 
        IFernflowerLogger.Severity.WARN);
      DecompilerContext.getLogger().writeMessage(
        "Class info - synthetic: " + cl.isSynthetic() + ", hasModifier(ACC_MODULE): " + cl.hasModifier(CodeConstants.ACC_MODULE), 
        IFernflowerLogger.Severity.WARN);
      DecompilerContext.getLogger().writeMessage(
        "Methods count: " + cl.getMethods().size(), 
        IFernflowerLogger.Severity.WARN);
    }
    ...
    try {
      ...
      if (packageInfo) {
        ...
        // Special debugging for l2/gameserver/model/Creature before LambdaProcessor
        if ("l2/gameserver/model/Creature".equals(cl.qualifiedName)) {
          DecompilerContext.getLogger().writeMessage(
            "=== DEBUG: About to process lambda methods for: " + cl.qualifiedName + " ===", 
            IFernflowerLogger.Severity.WARN);
          
          // Log bootstrap methods information
          if (cl.isVersion8()) {
            DecompilerContext.getLogger().writeMessage(
              "Class is Java 8+ compatible - checking for bootstrap methods", 
              IFernflowerLogger.Severity.WARN);
          }
        }
    ...
    public ClassNode(String content_class_name,
      ...
      if (!is_method_reference) { // content method in the same class, check synthetic flag
        ...
        if (mt != null) {
          ...
          // Log the issue and assume it's a method reference if method not found
          DecompilerContext.getLogger().writeMessage(
            "WARNING: Method '" + content_method_name + "' with descriptor '" + content_method_descriptor + 
            "' not found in class '" + classStruct.qualifiedName + "'. Assuming method reference.", 
            IFernflowerLogger.Severity.WARN);
