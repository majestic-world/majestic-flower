...
public class ConsoleDecompiler implements IBytecodeProvider, IResultSaver {
  ...
  public static void main(String[] args) {
    ...
    try {
      logger = new DebugFileLogger(System.out, "./logs.log");
    } catch (IOException e) {
      System.err.println("Warning: Could not create log file, falling back to console-only logging: " + e.getMessage());
      logger = new DebugFileLogger(System.out); // Console-only constructor
    }
    ...
    // Close the logger to flush any remaining logs
    if (logger instanceof DebugFileLogger) {
      ((DebugFileLogger) logger).close();
    }
  }
